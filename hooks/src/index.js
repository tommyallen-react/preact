...
options._commit = (internal, commitQueue) => {
	commitQueue.some(internal => {
		try {
			...
			commitQueue.some(i => {
				if (i.commitCallbacks) i.commitCallbacks = [];
			});
			commitQueue = [];
...
export function useLayoutEffect(callback, args) {
	...
	if (!options._skipEffects && argsChanged(state._args, args)) {
		...
		if (currentInternal.commitCallbacks == null) {
			currentInternal.commitCallbacks = [];
		}
		currentInternal.commitCallbacks.push(state);
	}
}
